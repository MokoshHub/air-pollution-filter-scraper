...
def download_data_by_date(date, data_path="pms_data"):
    ...
    for a in tqdm(a_tags):
        if "indoor" not in a.get_attribute("href") and "sds011" in a.get_attribute(
            "href"
        ):
            wget.download(
                a.get_attribute("href"), os.path.join(data_path, date), bar=None
            )
            ...
            if file.endswith(".csv"):
                with open(os.path.join(root, file)) as csv_file:
                    ...
                    for row in tqdm(single_sensor_file, leave=False):
                        with jsonlines.open(
                            os.path.join(data_path, "mika" + ".jsonl"), mode="a"
                        ) as writer:
                            sensor_id = row[0]
                            timestamp = row[5]
                            lat = row[3]
...
def clean_old_sensors(DATA_PATH):
    ...
    with jsonlines.open(
        os.path.join(DATA_PATH, "mika" + ".jsonl"), mode="w"
    ) as updater:
